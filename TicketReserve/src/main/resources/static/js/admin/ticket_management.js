      ...
      if (data != "") {
        ...
        $.each(data, function(index, perfomance) {
          ...
          if (perfomance.p_time == null || perfomance.p_ticket == null) {
            html += '<td>';
            html += '<button onclick="ticketRegistForm(\'' + perfomance.p_id + '\', \'' + perfomance.p_name.replace(/'/g, "\\'") + '\')">등록</button>';
            html += '</td>';
          }
          else {
            html += '<td>';
            html += '<button onclick="ticketModifyForm(\'' + perfomance.p_id + '\', \'' + perfomance.p_name.replace(/'/g, "\\'") + '\')">수정</button>';
            html += '</td>';
          }
...
function ticketRegistForm(p_id, p_name) {
	console.log("ticketModifyForm()");
	...
	$.ajax({
	    ...
	    success: function (data) {
	        if (data) {
	            ...
	            $('input[name="t_seattype_5"]').val(t5);
	            $('input[name="t_price_5"]').val(p5);
	            $('input[name="t_seattype_6"]').val(t6);
	            $('input[name="t_price_6"]').val(p6);
	            $('input[name="t_p_date"]').val(date);
	            
	            $('.regist_modal_wrap').css("display", "block");
			}
	    },
	    	error: function (xhr, status, error) {
		        console.log("AJAX 요청 실패: " + status);
...
$(document).on('click', '.ticket-button', function() {
	...
	$('input[name="t_seattype_5"]').val('');
	$('input[name="t_price_6"]').val('');
	$('input[name="t_p_dat6"]').val('');
...
function modifyConfirm() {
	...
	if (form.t_p_date.value == '') {
		alert("공연 날짜 및 시간을 입력해주세요.");
		form.t_p_date.focus();
		return;
	}
...
function regist_close() {
	$('.regist_modal_wrap').css("display", "none");
}
